...
def test_create_superuser_invalid_flags():
    password = secrets.token_urlsafe(12)
    with pytest.raises(ValueError):
        User.objects.create_superuser(email="badstaff@example.com", password=password, is_staff=False)
    with pytest.raises(ValueError):
        User.objects.create_superuser(email="badsuper@example.com", password=password, is_superuser=False)
...
def test_token_set_and_check_refresh_token():
    user = User.objects.create_user(email="token@example.com", password=secrets.token_urlsafe(12))
...
def test_token_parent_child_relationship():
    user = User.objects.create_user(email="parent@example.com", password=secrets.token_urlsafe(12))
...
def test_user_profile_auto_created_signal():
    user = User.objects.create_user(email="signal@example.com", password=secrets.token_urlsafe(12))
...
def test_user_profile_update_fields():
    user = User.objects.create_user(email="profile@example.com", password=secrets.token_urlsafe(12))
...
def test_token_unique_refresh_token_hash_constraint():
    user = User.objects.create_user(email="unique@example.com", password=secrets.token_urlsafe(12))
...
def test_token_unique_refresh_token_hash_constraint_violation():
    user = User.objects.create_user(email="repeat@example.com", password=secrets.token_urlsafe(12))

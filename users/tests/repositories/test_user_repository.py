...
class TestUserRepository:
    ...
    def test_get_user_by_email_success_and_not_found(self):
        user = User.objects.create_user(
            email="testemail@example.com", password="irrelevant"
        )
        found = self.repo.get_user_by_email(user.email)
    ...
    def test_get_user_by_id_success_and_not_found(self):
        user = User.objects.create_user(
            email="testid@example.com", password="irrelevant"
        )
        found = self.repo.get_user_by_id(user.id)
    ...
    def test_update_user_password_sets_time_and_password(self, generate_password):
        user = User.objects.create_user(
            email="updpw@example.com", password=generate_password()
        )
    ...
    def test_update_login_fail_count_success_and_trigger_lock(self, generate_password):
        ...
        # now와 비교해 offset-aware여야 함
        assert user.account_locked_until > timezone.now()
        assert user.is_account_locked()

    def test_check_email_exists_true_and_false(self, generate_password):
        email = "existcheck@example.com"
        User.objects.create_user(email=email, password=generate_password())
        assert self.repo.check_email_exists(email) is True
        assert self.repo.check_email_exists("absent@example.com") is False

    def test_delete_user_removes_user(self, generate_password):
        user = User.objects.create_user(
            email="deluser@example.com", password=generate_password()
        )
        self.repo.delete_user(user)
        with pytest.raises(User.DoesNotExist):
            User.objects.get(pk=user.pk)

    def test_get_user_profile_returns_none_or_profile(self, generate_password):
        user = User.objects.create_user(
            email="profilecheck@example.com", password=generate_password()
        )
    ...
    def test_get_user_confirmed_and_unconfirmed_2fa_devices(self, generate_password):
        user = User.objects.create_user(
            email="fauser@example.com", password=generate_password()
        )
    ...
    def test_create_2fa_device_creates_unconfirmed_device(self, generate_password):
        user = User.objects.create_user(
            email="devicecreate@example.com", password=generate_password()
        )

...
def test_authenticate_invalid_token(auth_request_factory):
    ...
    auth = JWTAuthentication()
    request = auth_request_factory.get('/', HTTP_AUTHORIZATION='Bearer invalid.token.string')
...
@pytest.mark.django_db
def test_authenticate_token_from_cookies_success(auth_request_factory, user_with_tokens):
...
def test_authenticate_with_expired_token(auth_request_factory, user_with_profile):
    ...
    auth = JWTAuthentication()
    request = auth_request_factory.get('/', HTTP_AUTHORIZATION=f'Bearer {expired_token}')

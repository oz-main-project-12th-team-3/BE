...
class TestChatAPI:
    ...
    def test_chat_message_create_and_list(self, authenticated_user):
        ...
        # 메시지 생성
        response = client.post(url, {'session': session.id, 'message': 'Hello, world!'}, format='json')
        assert response.status_code == status.HTTP_201_CREATED
    ...
    def test_cannot_access_others_session(self, authenticated_user):
        ...
        user2 = User.objects.create_user(email='otheruser@example.com', password='otherpassword')
        
        # user2가 세션 생성
        session_of_user2 = ChatSession.objects.create(user=user2, title="Other's Session")
        
        # user1이 user2의 세션에 메시지를 보내려고 시도 -> 실패해야 함
        message_url = reverse('chat-message-list-create')
        response = client1.post(message_url, {'session': session_of_user2.id, 'message': 'Hi there!'}, format='json')
        assert response.status_code == status.HTTP_400_BAD_REQUEST # serializer.is_valid()에서 걸림

        # user1이 user2의 메시지 목록을 보려고 시도 -> 빈 목록이 나와야 함
        response = client1.get(f'{message_url}?session_id={session_of_user2.id}')
        assert response.status_code == status.HTTP_403_FORBIDDEN # PermissionDenied 발생